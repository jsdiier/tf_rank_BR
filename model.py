...
class Model(tf.keras.Model):
    def __init__(self, training=False, pred=False, fid_kv=None, fid_ads_kv=None, l2_reg=0.0001):
        ...
        self.rankmixer = RankMixer(t=model_conf.rankmixer_token_count, token_dim=model_conf.rankmixer_token_dim,
                                   num_heads=model_conf.rankmixer_token_count, num_experts=1, hidden_ratio=2,
        ...
        self.rankmixer_seq_projections = [
            tf.keras.layers.Dense(model_conf.rankmixer_token_dim, kernel_regularizer=token_reg,
                                  name='rankmixer_sequence_{}_projection'.format(index))
    ...
    def call(self, inputs, training=None):
        ...
        rankmixer_tokens.extend([
            projection(seq_output)
            for projection, seq_output in zip(self.rankmixer_seq_projections, seq_outputs)
        ])
